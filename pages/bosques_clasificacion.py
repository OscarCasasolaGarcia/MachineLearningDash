...
layout = html.Div([
    html.H3('Bosques aleatorios 🌳🌲🌳 (Clasificación)'),
    dcc.Upload(
        id='upload-data',
        children=html.Div([
    ...
    return html.Div([
        ...
        dcc.Tabs([
            ...
            dcc.Tab(label='Aplicación del algoritmo', style=tab_style, selected_style=tab_selected_style, children=[
                ...
                dbc.Button("Click para obtener la clasificación", color="primary", className="mr-1", id='submit-button-clasificacion-bosques'),

                html.Hr(),

                # Mostramos la matriz de confusión
                html.H2(["", dbc.Badge("Matriz de clasificación", className="ms-1")]),
                dcc.Graph(id='matriz-bosque-clasificacion'),
...
@callback(
    Output('matriz-bosque-clasificacion', 'figure'),
...
def clasificacion(n_clicks, X_Clase, Y_Clase):
    if n_clicks is not None:
        ...
        fig = ff.create_annotated_heatmap(z=Matriz_Clasificacion.values, x=list(Matriz_Clasificacion.columns), y=list(Matriz_Clasificacion.index), colorscale='Viridis')
        ...
        # profundidad = ClasificacionBA.get_depth()
        # hojas = ClasificacionBA.get_n_leaves()
        # nodos = ClasificacionBA.get_n_leaves() + ClasificacionBA.get_depth()
